...
def DA_WithChannels_Add(inputimg):
    ...
    AUG_OPS_NAME = ["WithChannels_AddC123" , "WithChannels_AddC12", "WithChannels_AddC23",
                    ...
                    "WithChannels_AddC3"]
    AUG_OPS_PARAM = np.arange(-30, 30, 10)
    ret = []

    for i_param in np.arange(len(AUG_OPS_PARAM)):
        VALUE = AUG_OPS_PARAM[i_param]
        NameC012 = "DA_WithChannels_Add_" + "C012" + "_" + str(VALUE)
        imgC012 = iaa.WithChannels([0, 1, 2], iaa.Add(value=VALUE)).augment_image(inputimg)
        ret.append((NameC012, str(VALUE), imgC012))
        NameC01 = "DA_WithChannels_Add_" + "C01" + "_" + str(VALUE)
        imgC01 = iaa.WithChannels([0, 1], iaa.Add(value=VALUE)).augment_image(inputimg)
        ret.append((NameC01, str(VALUE), imgC01))

        NameC12 = "DA_WithChannels_Add_" + "C12" + "_" + str(VALUE)
        imgC12 = iaa.WithChannels([1, 2], iaa.Add(value=VALUE)).augment_image(inputimg)
        ret.append((NameC12, str(VALUE), imgC12))
        NameC02 = "DA_WithChannels_Add_" + "C02" + "_" + str(VALUE)
        imgC02 = iaa.WithChannels([0, 2], iaa.Add(value=VALUE)).augment_image(inputimg)
        ret.append((NameC02, str(VALUE), imgC02))

        NameC0 = "DA_WithChannels_Add_" + "C0" + "_" + str(VALUE)
        imgC0 = iaa.WithChannels(0, iaa.Add(value=VALUE)).augment_image(inputimg)
        ret.append((NameC0, str(VALUE), imgC0))
        NameC1 = "DA_WithChannels_Add_" + "C1" + "_" + str(VALUE)
        imgC1 = iaa.WithChannels(1, iaa.Add(value=VALUE)).augment_image(inputimg)
        ret.append((NameC1, str(VALUE), imgC1))
        NameC2 = "DA_WithChannels_Add_" + "C2" + "_" + str(VALUE)
        imgC2 = iaa.WithChannels(2, iaa.Add(value=VALUE)).augment_image(inputimg)
        ret.append((NameC2, str(VALUE), imgC2))
    ...
    # assert len(ret) == 70, "DA_WithChannels_Add output size not match"
    return ret
...
def DA_Superpixels(inputimg):
    ...
    for i in np.arange(len(Superpixels_param)):
        ...
        Name = "DA_Superpixels_" + str(VALUE[0]) + "_" + str(VALUE[1])
    ...
    assert len(ret) == 4, "DA_Superpixels output size not match"
    return ret

def DA_ColorspaceWithChannel(inputimg):
    assert inputimg.ndim in [2, 3], "input invalid! Please check input"
    # AUG_OPS_NAME = ["ColorspaceWithChannels_AddC23", "ColorspaceWithChannels_AddC2",
    #                 "ColorspaceWithChannels_AddC3"]
    AUG_OPS_PARAM = np.arange(-20,20,10)
    ret = []

    for i_param in np.arange(len(AUG_OPS_PARAM)):
        VALUE = AUG_OPS_PARAM[i_param]
        NameC12 = "DA_ColorspaceWithChannel_" + "C12" + "_" + str(VALUE)
        aug = iaa.Sequential([
            iaa.ChangeColorspace(from_colorspace="RGB", to_colorspace="HSV"),
            iaa.WithChannels([1, 2], iaa.Add(VALUE)),
            iaa.ChangeColorspace(from_colorspace="HSV", to_colorspace="RGB")
        ])
        imgC12 = aug.augment_image(inputimg)
        ret.append((NameC12, str(VALUE), imgC12))

        NameC1 = "DA_ColorspaceWithChannel_" + "C12" + "_" + str(VALUE)
        aug = iaa.Sequential([
            iaa.ChangeColorspace(from_colorspace="RGB", to_colorspace="HSV"),
            iaa.WithChannels(1, iaa.Add(VALUE)),
            iaa.ChangeColorspace(from_colorspace="HSV", to_colorspace="RGB")
        ])
        imgC1 = aug.augment_image(inputimg)
        ret.append((NameC1, str(VALUE), imgC1))

        NameC2 = "DA_ColorspaceWithChannel_" + "C12" + "_" + str(VALUE)
        aug = iaa.Sequential([
            iaa.ChangeColorspace(from_colorspace="RGB", to_colorspace="HSV"),
            iaa.WithChannels(2, iaa.Add(VALUE)),
            iaa.ChangeColorspace(from_colorspace="HSV", to_colorspace="RGB")
        ])
        imgC2 = aug.augment_image(inputimg)
        ret.append((NameC2, str(VALUE), imgC2))
...
def DA_Sharpen(inputimg):
    ...
    assert inputimg.ndim in [2, 3], "input invalid! Please check input"
    alpha = np.arange(0.0, 1.01, 0.25)
    lightness = np.arange(0.3, 1.31, 0.25)
    ret = []
    for i in np.arange(len(alpha)):
        for j in np.arange(len(lightness)):
            Name = "DA_Sharpen_" + str(alpha[i]) + "_" + str(lightness[j])
            VALUE = str(alpha[i]) + "_" + str(lightness[j])
            aug_img = iaa.Sharpen(alpha=alpha[i], lightness=lightness[j]).augment_image(inputimg)
            ret.append((Name, VALUE, aug_img))
    assert len(ret) == 25, "DA_Sharpen output size not match!"
    return ret

def DA_Emboss(inputimg):
    alpha = np.arange(0.0, 1.0, 0.25)
    strength = np.arange(0.3, 1.31, 0.25)
    assert inputimg.ndim in [2, 3], "input invalid! Please check input"
    ret = []
    for i in np.arange(len(alpha)):
        for j in np.arange(len(strength)):
            Name = "DA_Emboss" + str(alpha[i]) + "_" + str(strength[j])
            VALUE = str(alpha[i]) + "_" + str(strength[j])
            aug_img = iaa.Emboss(alpha=alpha[i], strength=strength[j]).augment_image(inputimg)
            ret.append((Name, VALUE, aug_img))
    assert len(ret) == 20, "DA_Sharpen output size not match!"
    return ret
...
def DA_MultiplyElementwise(inputimg):
    ...
    '''
    assert inputimg.ndim in [2, 3], "input invalid! Please check input"
    mul_values = np.arange(0.6, 2.0, 0.2)
...
def DA_Dropout(inputimg):
    assert inputimg.ndim in [2, 3], "input invalid! Please check input"
    values = np.arange(0.0,0.21,0.04)
    p_channels = np.arange(0.0,1.01,0.2)
...
def DA_CoarseDropout(inputimg):
    assert inputimg.ndim in [2, 3], "input invalid! Please check input"
    values = np.arange(0.0, 0.21, 0.05)
    p_channels = np.arange(0.0, 1.01, 0.2)
    ret = []
    for i in np.arange(len(values)):
        for j in np.arange(len(p_channels)):
            Name = "DA_CoarseDropout_" + str(values[i]) + "_" + str(p_channels[j])
            VALUE = str(values[i]) + "_" + str(p_channels[j])
...
def DA_Affine(inputimg):
    assert inputimg.ndim in [2, 3], "input invalid! Please check input"
    shear_values = np.arange(-10,10,1)
    rotate_values = np.arange(-10,10,1)
    trans_px = np.arange(-10, 10, 2)
    trans_py = np.arange(-10, 10, 2)
    ret = []
    ...
    for i in np.arange(len(trans_px)):
...
def DA_ElasticTransformation(inputimg):
    ...
    for i in np.arange(len(alpha_values)):
        ...
        Elastic_img = iaa.ElasticTransformation(alpha=alpha_values[i], sigma=sigma_values).augment_image(inputimg)
