...
# --*-- coding:utf-8 --*--
# Author: GK
# the main part implemented comes from github basic_keras_yolo, a very nice project
# Create on 18-3-14  上午10:58
...
from DataBatch import BatchGenerator
from keras.callbacks import EarlyStopping, ModelCheckpoint, TensorBoard
from YoloUtils import BoundBox
from FeatureExtractor import TinyYoloFeature, FullYoloFeature, MobileNetFeature, \
    SqueezeNetFeature, Inception3Feature, VGG16Feature, ResNet50Feature



class DLAFramework(object):
    def __int__(self, task_type,
                      architecture,
                      input_size,
                      labels,
                      max_box_per_image,
        ...
        elif self.task == "Classification_lenet":
            self.architecture = architecture
            self.input_size = input_size
            self.labels = labels
            self.nb_class = len(labels)
            self.control_param1 = control_param1
            self.control_param2 = control_param2

            # make the feature extractor layers
            input_image = Input(shape=(self.input_size, self.input_size, 3))
        else:
            print "task not supported now!"
        ...
        if self.task == "Detection_yoloV2":
            ...
            raise Exception('this task DLFramework not supported! '
                            'Only support Full Yolo, Tiny Yolo, MobileNet, SqueezeNet, VGG16,'
    ...
    def custom_loss(self, y_true, y_pred):
        if self.task == "Detection_yoloV2":
            ...
            true_box_xy, true_box_wh, coord_mask = tf.cond(tf.less(seen, self.warmup_bs),
                                                           lambda: [true_box_xy + (0.5 + cell_grid) * no_boxes_mask,
                                                                    true_box_wh + tf.ones_like(
                                                                        true_box_wh) * np.reshape(self.anchors,
                                                                                                  [1, 1, 1, self.nb_box,
                                                                                                   2]) * no_boxes_mask,
            ...
            print "not surpported at the moment!"
            return None
